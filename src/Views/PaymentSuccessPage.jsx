...
export const PaymentSuccessPage = () => {
  ...
  const { fetchCreateBooking } = useBookings();
  ...
  useEffect(() => {
    ...
    const processBooking = async () => {
      if (bookingData) {
        try {
          ...
          await fetchCreateBooking(mappedBookingData);
    ...
    processBooking();
  }, [bookingData, fetchCreateBooking]);
        ...
        <div className="space-y-4">
          ...
          <div className="bg-green-50 p-4 rounded border border-green-200 text-left">
            ...
            <div className="space-y-1 text-sm text-green-700">
              <p>
                <strong>Servicio:</strong> {getServiceName(bookingData)}
              </p>
              <p>
                <strong>Mascota:</strong> {getPetName(bookingData)}
              </p>
              <p>
                <strong>Sitter:</strong> {getSitterName(bookingData)}
              </p>
              <p>
                <strong>Fecha inicio:</strong>{" "}
                {dateFormatter.format(new Date(bookingData.startTime))}{" "}
                {timeFormatter.format(new Date(bookingData.startTime))}
              </p>
              <p>
                <strong>Fecha fin:</strong>{" "}
                {dateFormatter.format(new Date(bookingData.endTime))}{" "}
                {timeFormatter.format(new Date(bookingData.endTime))}
              </p>
              <p>
                <strong>Total pagado:</strong> ${bookingData.totalPrice}
              </p>
              {paymentMethod && (
                <p>
